...
from .models import Note, Param_stock_strategy_breakout
from . import db
import json
from sqlalchemy import desc, asc, func, select, and_, create_engine
import time
...
@views.route("/apply_strategy", methods=['GET', 'POST'])
@login_required
def apply_strategy():
    ...
    return redirect(url_for('views.strategy', strategy_name = strategy.name, mode = 'applied'))

@views.route("/strategies", methods = ['GET', 'POST'])
@login_required
def strategies():
...
@views.route("/strategy/<strategy_name>/<mode>")
@login_required
...
@views.route("/delete_traded_strategy", methods=['GET', 'POST'])
@login_required
def delete_traded_strategy():
    ...
    return redirect(url_for('views.strategy', strategy_name = strategy_name, mode = 'applied'))

@views.route("/apply_saved_strategies/<strategy_name>", methods=['GET', 'POST'])
@login_required
def apply_traded_strategy(strategy_name):
    ...
    for parsed in array_parsed:
        stock_id = array_parsed[parsed]['stock_id']
        parameter_id = parsed
        print(parameter_id)
    ...
    return redirect(url_for('views.strategy', strategy_name = strategy_name, mode = 'saved'))
